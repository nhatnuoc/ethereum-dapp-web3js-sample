...
"use strict";
angular.module("app.controllers", ['angular-loading-bar', 'ngTable','ui.bootstrap'])
...
.controller("AppCtrl", [
  "$scope", "$rootScope","$location", function($scope, $rootScope,$location) {
    ...
    if (userDataInlocal != '' && userDataInlocal != null && userDataInlocal != 'null' && userDataInlocal != 'undefined') {
      $rootScope.userData = JSON.parse(userDataInlocal);
    }
    console.log(JSON.stringify($rootScope.userData));
